...
function KnowledgePage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Hero — background photo + gradient overlay, matches /practice */}
      <section className="relative overflow-hidden bg-[color:var(--navy)] text-white">
        <img
          src="/assets/hero-knowledge-education.png"
          alt=""
          aria-hidden="true"
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-20 sm:py-28">
          ...
          <p className="mt-6 font-serif text-xl sm:text-2xl text-[color:var(--signal-soft)] max-w-3xl leading-snug">{t("knowledge.subtitle")}</p>
          ...
          <ol className="mt-12 space-y-8">
            {CHECKLIST_KEYS.map((item, i) => (
              <li key={item.title} className="grid grid-cols-[auto_1fr] gap-6">
                <span className="font-serif text-2xl text-[color:var(--alert)] leading-none pt-1 tabular-nums">{String(i + 1).padStart(2, "0")}</span>
                <div>
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {GUIDE_KEYS.map((g) => (
              <article key={g.title} className="relative flex flex-col rounded-md border border-white/10 bg-[color:var(--navy)] p-8 text-white">
                <div className="absolute inset-x-0 top-0 h-[3px] bg-[color:var(--signal)] rounded-t-md" />
                ...
                <Link to="/" hash="intake-form" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-[color:var(--signal-soft)] hover:text-[color:var(--signal)] transition">
      ...
      <section className="py-16 sm:py-24 bg-white">
        <div className="max-w-4xl mx-auto px-4">
          <div className="rounded-md border border-[color:var(--alert)]/30 bg-[color:var(--slate-dark)] text-white p-10 sm:p-14 text-center">
